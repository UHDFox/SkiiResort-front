...
// Function to get the list of skipass and display it
async function GetSkipassesList() {
    const x = document.getElementById("skipassesList");
    ...
    await fetchSkipasses();  // Fetch initial list of skipass
}

// Function to fetch skipass from API
async function fetchSkipasses() {
    if (isLoading) return;  // Prevent fetching if already loading
    isLoading = true;  // Set loading state

    try {
        // Fetch data from API
        const response = await fetch(`${config.apiUrl}/Skipass?offset=${offset.a}&limit=${limit.limit}`);
        ...
        if (data.list.length > 0) {
            ...
            // If no skipasses found, display a message
            document.getElementById('list').innerHTML = "<li>No skipasses found.</li>";
        }
...
function addToList(data) {
    ...
    listItem.textContent = `${data.id} - ${data.balance}`;  // Display both the ID and name
...
async function GetNextPage() {
    if (!isLoading) {  // Proceed if not already loading
        ...
        await fetchSkipasses();  // Fetch the next page of results
    }
}
...
async function GetPrevPage() {
    if (!isLoading && offset.a > 0) {  // Proceed if not already loading and not on the first page
        ...
        await fetchSkipasses();  // Fetch the previous page of results
    }
}
...
window.GetSkipassesList = GetSkipassesList;
