...
// Function to get the list of locations and display it
async function GetLocationsList() {
    const x = document.getElementById("locationsList");
    ...
    await fetchLocations();  // Fetch initial list of locations
}
...
async function fetchLocations() {
    ...
    try {
        // Fetch data from API
        const response = await fetch(`${config.apiUrl}/Location?offset=${offset.a}&limit=${limit.limit}`);
        ...
        if (data.list.length > 0) {
            ...
            // If no locations found, display a message
            document.getElementById('list').innerHTML = "<li>No locations found.</li>";
        }
...
async function GetNextPage() {
    if (!isLoading) {  // Proceed if not already loading
        ...
        await fetchLocations();  // Fetch the next page of results
    }
}
...
async function GetPrevPage() {
    if (!isLoading && offset.a > 0) {  // Proceed if not already loading and not on the first page
        ...
        await fetchLocations();  // Fetch the previous page of results
    }
}
...
window.GetLocationsList = GetLocationsList;
